...
public class HoleUtil implements MixinInterface {
    ...
    public static boolean isVoidHole(BlockPos blockPos) {
        return mc.player.dimension == -1 ? (blockPos.getY() == 0 || blockPos.getY() == 127) && mc.world.getBlockState(blockPos).getBlock().equals(Blocks.AIR) : blockPos.getY() == 0 && mc.world.getBlockState(blockPos).getBlock().equals(Blocks.AIR);
    }
